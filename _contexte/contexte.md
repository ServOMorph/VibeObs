...
## État actuel
- 2026-09-04 : le template `discord_com` ne déduplique plus à la seconde ; `notify` attend la prise en charge du bot. Tests isolés passants, test Discord réel en attente d’autorisation système. Kit v4.7.
- 2026-09-04 : la base `DOCUMENTATION/` dispose d'un contrôle mécanique (`check_docs.py`) exécuté par `/doc_sync`, et les `/close` signalent les informations durables à l’agent Documentation. Kit v4.6.
- 2026-09-04 : le protocole Roadmap impose désormais de proposer l'archivage à l'utilisateur après achèvement de toutes les phases, sans archivage automatique. Kit v4.4.
- 2026-09-03 : correctif template `discord_com` (perte d'un message sur deux + latence à vide) et guidage Bot Token renforcé ; `/create_agent` propose son insertion pour un agent Discord. Kit v4.3.
- 2026-08-31 : `/create_projet_public` crée un projet vierge sous `PROJETS_PARENT_DIR`, avec dépôt GitHub optionnel, puis `/init_projet`.
