...
`/start`, `/close`, `/init_projet`, `/update`, `/create_memory` (déployées dans les projets) ; kit uniquement : `/create_agent`, `/create_team`, `/create_parallel_team`, `/create_com_agents`, `/insert_template`, `/init_discord_mode`, `/init_intercom`, `/cherche_meilleure_action`, `/doc_sync`, `/cherche_fonction`. Les équipes utilisent `team.md` et relaient les statuts/messages uniquement entre parents et enfants directs. `/create_parallel_team` ajoute un worktree et une branche par membre ; seule une validation explicite autorise l'intégration. Intercom relie des projets distincts via des boîtes de réception locales. Détails : README et guides 20_.
