...
## 2026-09-02 — agents design + discord (Appli_TSA_SDI_TDAH)
Création en lot (2 agents) depuis une seule invocation. Les deux rôles étaient fournis en argument mais courts ("il va s'occuper du desing de l'appli", faute de frappe incluse) — reformulés en rôle durable sans invention (design d'appli / spécialiste automation Discord), pas d'aller-retour nécessaire sur ce point.
Un seul aller-retour groupé (AskUserQuestion, 2 questions) sur le périmètre d'écriture : DESIGN étendu à `src/ui/` + `src/app/` (rôle touchant le code d'interface), DISCORD étendu à `scripts/`. Le périmètre par défaut aurait été incohérent avec un rôle qui "s'occupe du design de l'appli".
A bien fonctionné : `start.md` du projet charge déjà `agent_role.md` (étape 3b, pas de warning). Zone racine déterminable (`Appli_TSA_SDI_TDAH` → racine), `{{ALIAS_PARENT}}` fiable. Aliases `design` et `discord` absents de `zones.md` → mode création propre. Stack DESIGN filtrée depuis package.json + README (React 19/Vite 8/Dexie, contrainte accessibilité AuDHD, budget bundle). Stack DISCORD : aucune infra Discord trouvée (grep), "hérite de la stack parente" + pointeur vers le template kit `discord_com/` / skill `init_discord_mode`.

## 2026-09-02 — suite : template Discord + évolution de la commande
Après création, l'utilisateur a demandé d'insérer le template `templates/discord_com/` dans l'agent `discord` d'Appli_TSA_SDI_TDAH et d'ajouter à `create_agent.md` une question systématique sur ce point pour tout agent Discord.
Fait : template copié dans `DISCORD/discord_com/` (14 fichiers, copie brute, aucun placeholder), `config_bot_discord.json` (`enabled: false`) et `.env` créés depuis les exemples, `.env` bien couvert par le `.gitignore` du projet (pattern `.env`). Configuration bot renvoyée à `/init_discord_mode` (actions Developer Portal impossibles en session).
Friction relevée : le template embarque `.claude/commands/discord_loop.md`. Placé sous `DISCORD/discord_com/.claude/commands/`, ce slash command n'est pas chargé par Claude Code (il faut `.claude/commands/` à la racine du projet, hors périmètre agent). Signalé, décision laissée à l'utilisateur — pas de copie automatique à la racine.
Évolution `create_agent.md` : bullet ajouté en [COLLECTE] (question groupée, défaut oui pour dossier `DISCORD` ou rôle mentionnant Discord/bot) + bloc en [ECRITURE] (insertion brute, création config+.env depuis exemples, renvoi à `/init_discord_mode`, avertissement `/discord_loop`). À répercuter dans la doc du kit via `/doc_sync`.
